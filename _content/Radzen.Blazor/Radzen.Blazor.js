...
window.Radzen = {
  ...
  upload: function (fileInput, url, multiple) {
    ...
    for (var i = 0; i < fileInput.files.length; i++) {
      var file = fileInput.files[i];
      if(Radzen.filesToSkipOnUpload && Radzen.filesToSkipOnUpload.indexOf(file.name) != -1) continue;
  ...
  openPopup: function (parent, id, syncWidth, position) {
